...
describe('Custom Tests', function () {
  ...
  it('[assignment] ii. deposit 0.1 ETH in L1 -> withdraw 0.08 ETH in L2 -> assert balances', async () => {
      ...
      const tornadoPollBalance = await token.balanceOf(tornadoPool.address)
      expect(recipientBalance).to.be.equal(utils.parseEther('0.08'))
      expect(omniBridgeBalance).to.be.equal(0)
      expect(tornadoPollBalance).to.be.equal(utils.parseEther('0.02'))
  })
  ...
  it('[assignment] iii. see assignment doc for details', async () => {
      ...
      //console.log("0.13", utils.parseEther("0.13"))
      // const omniBridgeBalance = await token.balanceOf(omniBridge.address)
      // const tornadoPollBalance = await token.balanceOf(tornadoPool.address)
      // console.log(omniBridgeBalance)
      // console.log(tornadoPollBalance)
      ...
      //Alice withdraw in L1
      const aliceWithdrawAmount = utils.parseEther('0.07')
      const recipient = '0xCeD6b596d9C96E2C221b50fcE0d0685F9d1C09B0'
      ...
      const tornadoPollBalance = await token.balanceOf(tornadoPool.address)
      expect(tornadoPollBalance).to.be.equal(0)
  })
})
